(function(root){
'use strict';
const D=typeof module!=='undefined'?require('./data.js'):root.ConstructionData;
const KEY='construction-methods-v01';
...
function restore(raw){try{const st=JSON.parse(raw);if(!st||st.schemaVersion!==1||typeof st.sound!=='boolean'||typeof st.unlocked!=='boolean'||!integer(st.streak)||st.streak>2||st.streak===2&&!st.unlocked||!st.completed||!['basic','guided','challenge','random','review'].every(k=>integer(st.completed[k]))||!st.stats||Object.entries(st.stats).some(([id,v])=>![...D.questions,...D.basics].some(q=>q.id===id)||!v||!['first','retry','help'].every(k=>integer(v[k]))))throw 0;
 ...
 const ids=(learningMode(s)==='basic'?D.basics:D.questions).map(q=>q.id);if(s.order.some(id=>!ids.includes(id))||s.mode!=='review'&&s.order.length!==ids.length||!s.results||Array.isArray(s.results))throw 0;
...
const api={KEY,fresh,start,learningMode,item,prompt,answer,advance,help,restore};if(typeof module!=='undefined')module.exports=api;else root.ConstructionEngine=api;
})(globalThis);
