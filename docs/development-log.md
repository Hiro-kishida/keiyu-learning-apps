...
A案の成功版 `app_A_stage.html` は残したまま、少し難しい版として `app_A_stage_plus.html` を新規作成した。

### 方針

* 既存の `app_A_stage.html` は変更しない
* A案は成功版として残す
* A+案は「少しむずかしい練習」として追加する
* 問題数は10問のまま
* 難化の軸は「係数1・係数-1の省略」に絞る
* 生徒の成功体験を壊さず、1段だけ難しくする

### A+版の設定

`app_A_stage_plus.html` では、Spreadsheet記録用の `mode` を `A_PLUS` にした。

これにより、Spreadsheet上で通常のA案とA+案を区別できるようになった。
...
ねらいは、答えが `1x` になるときに `x` と書くこと、答えが `-1x` になるときに `-x` と書くことに慣れること。
...
GitHub Pages版でも動作確認済み。

Spreadsheetにも `mode: A_PLUS` で記録されることを確認した。

確認できたこと：

* 全10問正解 → `correctCount: 10`, `allCorrect: TRUE`
* `10x - 9x` を間違えた場合 → `wrongProblems` に `10x - 9x` が記録される
* `8x - 9x` を間違えた場合 → `wrongProblems` に `8x - 9x` が記録される

### index.html の更新

入口ページ `index.html` に、A+案へのリンクを追加した。
...
`係数1・係数-1の省略に少し強くなる練習です。`

確認できたこと：

* A案リンクは今まで通り開く
* A+案リンクが表示される
* A+案をクリックすると `app_A_stage_plus.html` が開く
* B案・C案リンクも壊れていない

### PM視点の評価

今回の重要点は、成功版を上書きしなかったこと。

境界知能寄りの生徒くんがA案で連続100点を取っているため、A案は成功体験を作る安定版として残した。

そのうえで、A+案を別ファイルとして作り、少しだけ難化した。

これにより、A+案が難しすぎた場合でも、すぐにA案へ戻れる。

今後の基本方針として、

* 成功版は残す
* 少し難しくする場合は別版を作る
* `mode` を分けてSpreadsheetで記録する
* 入口ページから選べるようにする
